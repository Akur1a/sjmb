<template>
  <div style="width:1572px">
    <div v-for="(item,index) in pageList"
      ...
      <!--装订线-->
      <div class="dashedLine" v-if="(index+1-1)%4 ==0">
      ...
      <div id="new-upload-header"
           ...
           v-if="index==0">
        <div id="header-title"
             style="margin-left: -22px;">南宁学院{{titleInfo.xymc}}考试(查)试卷
        </div>
        <div style="height: 32px"></div>
        <div class="header-content1"
             style="margin:0 auto">
          <div class="header-content-item">
            <div class="header-content-item-right">《<span
              style="min-width:510px;text-align:center;display:inline-block">{{titleInfo.kcmc}}</span>》课程
            </div>
          </div>
          <div class="noWidth">
        ...
        <div class="headTable">
          <div class="headTableRow">
            <div class="headTableCol1"
                 ref="njzy"
                 style="width:80px">年级专业
            </div>
            <div class="headTableCol1"
                 ref="njzyRes"
                 style="width:300px">{{titleInfo.grade}}
            </div>
            <div class="headTableCol1"
                 ref="mtjs"
                 style="width:80px">命题教师
            </div>
            <div class="headTableCol1"
                 ref="mtjsRes"
                 style="border-right:none;width:164px">{{titleInfo.mtjs}}
            </div>
            <div style="clear:both"></div>
          </div>
          <div class="headTableRow"
            ...
            <div class="headTableCol1"
                 style="width:80px">考核时长
            </div>
            <div class="headTableCol1"
                 style="border-right:none;width:164px"><span style="text-decoration: underline">&nbsp;&nbsp;{{titleInfo.kssc}}&nbsp;&nbsp;</span>分钟
            </div>
            <div style="clear:both"></div>
          </div>
        </div>
      ...
      <div style="margin-top:5px;"
           :style="[{height:index==0?'543px':'908px'},
           {marginLeft:index===0 || (index+1-1)%4 ==0?'125px':(index+1)%4 ==0?'5px':'31px'},
           {marginRight:(index+1)%4 ==0?'125px':'0'},
           {width:index===0 || (index+1-1)%4 ==0 || (index+1)%4 ==0?'630px':'724px'}]"
           ref="page"
           class="STArea"></div>
      <!-- 卷尾 -->
      <div style="font-size:16px">
        ...
        <div class="signArea"
             style="width:630px;
             margin-left: 125px;display:flex;justify-content:space-around"
  ...
  export default {
    ...
    methods: {
      ...
      putDTSM(val, count, nextTxt) {
        // 切分放置大题说明
        // let pageWidth = this.pageNo===0 || (this.pageNo+1-1)%4 ==0 || (this.pageNo+1)%4 ==0 ? '630':'724'//双面724
        let pageWidth = this.pageNo===0 || (this.pageNo+1-1)%4 ==0 || (this.pageNo+1)%4 ==0 ? '630':'724'
        let pureTxt = val
        let txt = `<p style="width:${pageWidth}px;margin-top: 6px;font-weight: 700;text-indent: 110px;font-size:18px">${val}</p>`
        let txt2 = `<p style="width:${pageWidth}px;margin-top: 6px;font-weight: 700;text-indent:30px;font-size:18px">${val}</p>`
        ...
        if (this.pageList[this.pageNo].height >= domHeight1 - 27) {
          ...
          if (nextTxt) {
            ...
            this.pageNo++
            pageWidth = this.pageNo===0 || (this.pageNo+1-1)%4 ==0 || (this.pageNo+1)%4 ==0 ? '630':'724'//双面724
            ...
            let domTxt1 = "<div style='width:" + pageWidth + "px;margin:0 auto'>" + newPage + '</div>'

            let creatDiv1 = document.createElement("div");
            // creatDiv1.style.width = "630px";
            creatDiv1.style.width = pageWidth+"px";
      ...
      inputFn(htmlTxt, pureTxt) {
        ...
        //生成虚拟dom,获取富文本高度
        let pageWidth = this.pageNo===0 || (this.pageNo+1-1)%4 ==0 || (this.pageNo+1)%4 ==0 ? '630':'724'//双面宽724
        let creatDiv = document.createElement("div");
        // creatDiv.style.width = "630px";
        creatDiv.style.width = pageWidth+"px";
        ...
        if (domHeight < this.pageList[this.pageNo].height) {
          ...
          let domTxt = "<div style='width:" + pageWidth +"px;margin:0 auto;font-size:18px'>" + htmlTxt + '</div>'
          ...
          for (let index = 0; index < domArr.length; index++) {
            ...
            if (listItemHeight < this.pageList[this.pageNo].height) {
              ...
              let domTxt1 = "<div style='width:" + pageWidth +"px;margin:0 auto;font-size:18px'>" + domArr[index].outerHTML + '</div>'
      ...
      inputFn1(txt, pureTxt, count, nextTxt) {
        ...
        creatDiv.style.display = "none";
        pageWidth = this.pageNo===0 || (this.pageNo+1-1)%4 ==0 || (this.pageNo+1)%4 ==0 ? '630':'724'//双面宽724

        // let domTxt = "<div style='width:630px;margin:0 auto;font-size:18px'>" + txt + '</div>'
        let domTxt = "<div style='width:"+pageWidth+"px;margin:0 auto;font-size:18px'>" + txt + '</div>'
        ...
        if (eachXtHeight < this.pageList[this.pageNo].height) {
          ...
          if (nextTxt) {
            ...
            let domTxt1 = "<div style='width:"+pageWidth+"px;margin:0 auto;font-size:18px'>" + newPage + '</div>'
      ...
      async inputDT(count, max, obj) {
        ...
        if (obj[index].sm) {
          ...
          this.QTBackup = {
            ...
            txt: `<p style="width:${pageWidth}px;font-weight: 300;font-size:18px;font-weight:700">${obj[index].sm}</p>`
          }
      ...
      async inputXT(count, max, obj) {
        ...
        if (obj.children[i].stlx == "single_select" || obj.children[i].stlx == "multi_select") {
          ...
          let switcher1 = XxArr.some((item) => {
            // return item >= 630 / 4
            return item >= pageWidth / 4

          })
          if (switcher1) {
            switcher2 = XxArr.some((item) => {
              // return item >= 630 / 2
              return item >= pageWidth / 2
        ...
        if (this.pageList[this.pageNo].height > 10) {
          ...
          if (this.$refs.page[this.pageNo]) {
            ...
            this.$refs.page[this.pageNo].innerHTML += `<div style='width:${pageWidth}px;margin:0 auto;font-size:18px;height:10px'></div>`
      ...
      async inputLine(count, max, val) {
        ...
        pageWidth = this.pageNo===0 || (this.pageNo+1-1)%4 ==0 || (this.pageNo+1)%4 ==0 ? '630':'724'
        if (this.$refs.page[this.pageNo]) {
          // this.$refs.page[this.pageNo].innerHTML += `<div style='width:630px;margin:0 auto;font-size:18px;height:27px'></div>`
          this.$refs.page[this.pageNo].innerHTML += `<div style='width:${pageWidth}px;margin:0 auto;font-size:18px;height:27px'></div>`
          ...
          this.$nextTick(() => {
            ...
            // this.$refs.page[this.pageNo].innerHTML += `<div style='width:630px;margin:0 auto;font-size:18px;height:27px'></div>`
            this.$refs.page[this.pageNo].innerHTML += `<div style='width:${pageWidth};margin:0 auto;font-size:18px;height:27px'></div>`
      ...
      async inputXX(count, max, obj) {
        ...
        if (this.$refs.page[this.pageNo]) {
          ...
          this.$refs.page[this.pageNo].innerHTML += "<div style='width:"+pageWidth+"px;margin:0 auto;font-size:18px'>" + obj[count] + '</div>'
      ...
      treeData: {
        ...
        handler(newValue, oldValue) {
          setTimeout(() => {
            ...
            this.pageList = [{
              width: pageWidth,
              height: 908 - this.$refs.pageHead[0].clientHeight,
              showDashedLine: true,
            }]
            this.pageNo = 0
            this.$nextTick(() => {
